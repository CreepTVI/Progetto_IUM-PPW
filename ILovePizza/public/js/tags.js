$(document).ready(function () {
    ...
    if(tagsRecived){
        tagsRecived.forEach(tag => {
            addTagToContainer(tagsInput, tagsContainer, valueContainer, tag.name);
        });
    }
        ...
        if (event.code === "Space" || event.code === "Spacebar" || event.keyCode === 32) {
            ...
            if (inputValue !== "") {
                ...
                if (tagsContainer.find(".tag-circle").length < maxTags) {
                    addTagToContainer(tagsInput, tagsContainer, valueContainer, inputValue);
                } else {
